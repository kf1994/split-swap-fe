...
export class BaseService {
      ...
      async (error) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          ...
          try {
            ...
            const { data } = await this.refreshAuthToken(refreshToken)
            const newToken = data?.data

            // Set the new token
            const decodedToken = jwt.decode(newToken) as TokenData
            if (decodedToken?.exp) {
              // Convert exp from unix timestamp (seconds) to Date object
  ...
  private refreshAuthToken(refreshToken: string) {
    return axios.post("/user/refresh-token", { refreshToken }, { baseURL: API_HOST })
  }
