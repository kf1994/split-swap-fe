...
import { type AnchorProvider, type Idl, Program, type ProgramAccount, web3 } from "@coral-xyz/anchor"
import { type NodeWallet, type Pool, type Position, type ITrendingCoinsResponse, type TokenInfo } from "@types"
...
import { DELEGATE_OPERATOR, DELEGATE_OPERATOR_ADDRESS, LOWER_THRESHOLD_PERCENT, PROFIT_FEE, SOL_ADDRESS } from "@config"
import { ReferralProvider, PROGRAM_ID as JUP_REFERRAL_PROGRAM_ID } from "@jup-ag/referral-sdk"
...
export class LavarageService extends BaseService {
  ...
  async getTrendingCoins() {
    return (await this.http.get<ITrendingCoinsResponse>(`${API_HOST}/tokens/trending`))?.data?.data
  }
  ...
  async getPools(): Promise<Pool[]> {
    ...
    const nodeWalletsMap = uniqueNodeWallets.reduce((acc: Record<string, NodeWallet>, wallet) => {
  ...
  async getPoolByTokenAddress(tokenAddress: string) {
    ...
    return getRelatedPool({ address: tokenAddress } as TokenInfo, pools, SOL_ADDRESS)
  }

  async getPosition(publicKey: string) {
    const positions = await this.program.account.position.all([{ dataSize: 178 }])

    return positions.filter((f) => f.publicKey.toBase58() === publicKey)?.[0] as ProgramAccount<{
      pool: PublicKey
      seed: PublicKey
      userPaid: BN
      amount: BN
  ...
  async getUserPosition(userAddress: string) {
    ...
    return positions.filter((f: any) => f.account.trader.toBase58() === userAddress) as ProgramAccount<{
      pool: PublicKey
      seed: PublicKey
      userPaid: BN
      amount: BN
  ...
  async openBorrowingPosition(
    ...
    const jupiterIxs = [...setupInstructions.map(deserializeInstruction), deserializeInstruction(swapInstructionPayload)]
  ...
  async partialRepay(percentage: number, poolKey: string, seed: PublicKey, borrowedAmount: number) {
        ...
        ...[
          borrowedAmount > 0.05
            ? web3.SystemProgram.transfer({
                ...
                lamports: new BN(borrowedAmount * LAMPORTS_PER_SOL).mul(new BN(1)).div(new BN(100)).toNumber()
              })
            : []
        ]
  ...
  async repaySol(poolKey: string, borrowedAmount: number, seed: PublicKey, collateralAmount: number, pnl: number) {
    ...
    const jupiterQuote = await jupiterSource.getPrice(tokenAddressPubKey.toBase58(), SOL_ADDRESS)
    ...
    const messageV0 = new TransactionMessage({
      ...
      instructions: [
        ...
        web3.SystemProgram.transfer({
          fromPubkey: this.program.provider.publicKey!,
          toPubkey: new PublicKey(FEE_RECIPIENT),
          lamports: pnl > 0 ? new BN(pnl * LAMPORTS_PER_SOL).mul(new BN(PROFIT_FEE)).div(new BN(100)).toNumber() : 0
        }),
        ...[
          borrowedAmount > 0.1
            ? web3.SystemProgram.transfer({
  ...
  async createTakeProfit(price: BigNumber, poolKey: string, seed: PublicKey) {
    ...
    const { blockhash } = await this.program.provider.connection.getLatestBlockhash("finalized")
    const positionAccountPDA = getPda(
      [Buffer.from("position"), this.program.provider.publicKey?.toBuffer(), poolPubKey.toBuffer(), seed.toBuffer()],
